...
function theme_move_get_extra_scss($theme) {
    ...
    $scss .= theme_move_set_headerimg($theme);

    $scss .= theme_move_set_topfooterimg($theme);

    $scss .= theme_move_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_move_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_move_set_topfooterimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_move_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.move-login #page-wrapper #page {background-image: url('$loginbgimg');}";
...
function theme_move_get_main_scss_content($theme) {
    ...
    if ($filename == 'default.scss') {
        ...
        // This preset file was fetched from the file area for theme_move and not theme_boost (see the line above).
    ...
    $move = file_get_contents($CFG->dirroot . '/theme/move/scss/move.scss');
 ...
 * @return string
 */
function theme_move_get_setting($setting, $format = false) {
    $theme = theme_config::load('move');
...
/**
 * Extend the Move navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_move_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_move_add_certificatesmenuitem($flatnav);

    theme_move_delete_menuitems($flatnav);

    theme_move_add_coursesections_to_navigation($flatnav);

    theme_move_rename_menuitems($flatnav);
}
 ...
 *
 */
function theme_move_add_certificatesmenuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        if (!theme_move_has_certificates_plugin()) {
            return;
        }

        $actionurl = new \moodle_url('/theme/move/certificates.php');
        ...
        if ($COURSE->id > 1) {
            ...
            $actionurl = new \moodle_url('/theme/move/certificates.php', ['id' => $COURSE->id]);
        }
        ...
        if (!is_null($parentitem->parent)) {
            $certificatesitemoptions = [
                'action' => $actionurl,
                'text' => get_string('certificates', 'theme_move'),
                'shorttext' => get_string('certificates', 'theme_move'),
 ...
 * @param flat_navigation $flatnav
 */
function theme_move_delete_menuitems(\flat_navigation $flatnav) {
    ...
    foreach ($flatnav as $item) {
        ...
        if ($item->key === 'mycourses') {
            foreach ($item->children as $key => $child) {
                if (!theme_move_is_course_available_to_display_in_navbar($child->key)) {
 ...
 * @return bool
 */
function theme_move_is_course_available_to_display_in_navbar($courseid) {
    global $DB, $USER;
 ...
 * @param flat_navigation $flatnav
 */
function theme_move_rename_menuitems(\flat_navigation $flatnav) {

    $item = $flatnav->find('mycourses');

    if ($item) {
        $item->text = get_string('myactivecourses', 'theme_move');
    }
}
 ...
 * @param flat_navigation $flatnav
 */
function theme_move_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_move'),
            'shorttext' => get_string('coursesections', 'theme_move'),
 ...
 * @return bool
 */
function theme_move_has_certificates_plugin() {
